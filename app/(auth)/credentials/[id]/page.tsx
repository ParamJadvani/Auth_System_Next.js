...
export default function AddCredentialsPage() {
    const { register, handleSubmit, reset } = useForm<CredentialFormValues>();
    ...
    const handleRemoveCustom = (index: number) =>
        setAdditional(additional.filter((_, i) => i !== index));
    ...
    const onSubmit = async (form: CredentialFormValues) => {
        ...
        // 7. Call addCredentials with FormData and user ID
        await updateCredentials(formData as unknown as CredentialFormValues, form.id as number, user?.user?.id as number);
    ...
    // If editing, fetch credential detail and populate form + state
    useEffect(() => {
            ...
            try {
                ...
                if (detail) {
                    ...
                    reset({
                        ...
                        tag_ids: detail.tags ? detail.tags.map((t: Tag) => t.id) : [],
                        files: [] as any, // leave files empty; uploads not retroactively populated
                        id: detail.id,
                    });

                    // Populate URL inputs
                    setWebsites(detail.website && detail.website.length > 0 ? detail.website : [""]);
                    ...
                    // Populate selectedTags from detail.tags
                    setSelectedTags(detail.tags ? detail.tags.map((t: Tag) => t.name) : []);

                    // You could populate selectedFiles if you show existing attachments, but typically
                    // editing does not re-show previous files for re-upload.
                }
            } catch (err) {
                console.error("Failed to fetch credential detail:", err);
            }
        };
        ...
        <Card className="mx-auto bg-white dark:bg-gray-800 shadow-none rounded-none">
            <CardHeader>
                <CardTitle className="text-2xl font-bold">
                    Edit Credentials <Button onClick={() => setEdit(!edit)} variant={edit ? "default" : "outline"} size="icon">Edit</Button>
                </CardTitle>
            </CardHeader>
            <CardContent>
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                        <IconInput id="name" label="Name *" placeholder="e.g., My Bank" {...register("name")} disabled={!edit} />
                        <IconInput
                            id="username"
                            label="Username *"
                            ...
                            disabled={!edit}
                        />
                        <PasswordInput id="password" label="Password *" type="password" {...register("password")} disabled={!edit} />
                        {edit && <PasswordInput id="two_factor_secret" label="Two Factor Secret" {...register("two_factor_secret")} disabled={!edit} />}
                    </div>
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between items-center">
                            <Label>URLs</Label>
                            <Button type="button" size="sm" onClick={handleAddUrl} variant="outline" disabled={!edit}>
                    ...
                    <div>
                        <Label>Notes</Label>
                        <Textarea placeholder="Any additional information" rows={3} {...register("notes")} disabled={!edit} />
                    </div>
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between items-center">
                            ...
                            <Button type="button" size="sm" onClick={handleAddCustom} variant="outline" disabled={!edit} >
